...
:root{
  ...
  --glass:#020617f0;--road:#020617;--lane:#1f2937;--line:#e5e7eb;
}
...
h1{
  margin:0;font-size:1.6rem;letter-spacing:.16em;text-transform:uppercase;
  text-shadow:0 0 10px rgba(56,189,248,0.9),0 0 22px rgba(56,189,248,0.6);
}
.subtitle{
  margin-top:4px;font-size:.8rem;color:var(--text-muted);letter-spacing:.08em;
}
...
.pill small{color:var(--text-muted);}
.pill select{
  background:transparent;border:none;color:var(--text-main);
...
/* layout */
.main-layout{
  display:grid;grid-template-columns:3fr 1.4fr;gap:12px;margin-top:12px;
}
...
.track{
  width:320px;height:460px;border-radius:18px;border:1px solid rgba(55,65,81,0.9);
...
/* road */
.road{
  position:absolute;top:0;bottom:0;left:18px;right:18px;
  background:var(--road);
  overflow:hidden;border-radius:16px;border:1px solid rgba(31,41,55,0.9);
}
.road::before{
...
/* lane markings (only vertical center, lanes drawn via JS) */
.lane-line{
  position:absolute;width:6px;height:54px;background:var(--line);
  left:50%;transform:translateX(-50%);
  opacity:0.75;border-radius:999px;
  animation:laneMove 0.7s linear infinite;
}
.lane-line:nth-child(2){animation-delay:0.23s;}
.lane-line:nth-child(3){animation-delay:0.46s;}
@keyframes laneMove{
  0%{top:-60px;}
  100%{top:460px;}
}
...
/* car base */
.car{
  position:absolute;width:42px;height:74px;border-radius:12px;
...
.car::before{
  content:"";width:26px;height:22px;border-radius:10px 10px 8px 8px;
  background:linear-gradient(180deg,#020617,#1f2937);
  margin-top:4px;border:1px solid rgba(15,23,42,0.9);
}
.car::after{
...
/* lights */
.car span{
  width:9px;height:9px;border-radius:4px;margin-top:auto;margin-bottom:6px;
  background:#facc15;box-shadow:0 0 10px rgba(250,204,21,0.9);
}
.car.enemy span{
  background:#f97316;box-shadow:0 0 10px rgba(248,113,113,0.9);
}
...
/* HUD */
.hud-box{
  border-radius:14px;padding:8px 10px 10px;background:rgba(15,23,42,0.9);
  border:1px solid rgba(55,65,81,0.9);margin-bottom:8px;
}
.hud-title{
...
.hud-label{color:var(--text-muted);font-size:.8rem;}
.hud-value{font-weight:600;color:#facc15;text-shadow:0 0 10px rgba(250,204,21,0.8);}
.speed-bar{
  width:100%;height:8px;border-radius:999px;background:#020617;border:1px solid rgba(55,65,81,0.9);
  overflow:hidden;margin-top:4px;
}
.speed-fill{
  height:100%;width:30%;border-radius:inherit;
...
.btn{
  border:none;border-radius:999px;padding:7px 16px;font-size:.8rem;font-weight:600;
...
.btn:hover{
  transform:translateY(-1px);box-shadow:0 0 14px rgba(56,189,248,0.6);
...
/* overlay */
.overlay{
  position:absolute;inset:0;background:rgba(15,23,42,0.93);
  display:flex;justify-content:center;align-items:center;
  visibility:hidden;opacity:0;transition:opacity .2s ease-out;
}
.overlay.show{visibility:visible;opacity:1;}
.overlay-card{
  background:#020617;border-radius:14px;padding:14px 18px 16px;
  border:1px solid rgba(248,113,113,0.7);max-width:280px;text-align:center;
  box-shadow:0 0 20px rgba(248,113,113,0.7);
}
.overlay-card h2{
  margin:0 0 6px;color:#f97316;text-transform:uppercase;letter-spacing:.14em;
  font-size:.9rem;
}
.overlay-card p{
  margin:4px 0 10px;font-size:.82rem;color:var(--text-muted);
}

/* keys */
.key-row{margin-top:6px;font-size:.76rem;color:var(--text-muted);}
.key-tag{
  display:inline-flex;align-items:center;justify-content:center;
  min-width:18px;height:18px;border-radius:4px;border:1px solid rgba(55,65,81,0.9);
    ...
    <div class="main-layout">
      <div class="track-shell">
        <div class="track">
          <div class="road" id="road">
            ...
            <!-- dynamic lanes are virtual via JS, enemies/coins injected -->
          </div>
      ...
      <div>
        <div class="hud-box">
          ...
          <div class="hud-row" style="margin-top:6px;">
            <div>
              ...
              <div style="font-size:.8rem;">
                <span id="speed-label">120</span> km/h
              </div>
            </div>
            <div>
              <div class="hud-label">Best score</div>
              <div style="font-size:.8rem;">
                <span id="best-label">0</span>
              </div>
            </div>
          </div>
          <div class="speed-bar">
          ...
          <div class="lives-row">
            Lives
            <span class="lives-hearts" id="lives-hearts">❤️❤️❤️</span>
          </div>
        </div>
        ...
        <div class="hud-box">
          ...
          <div class="btn-row">
            <button class="btn btn-primary" id="start-btn">Start race ▶️</button>
            <button class="btn" id="reset-btn">Reset stats ♻️</button>
          </div>
          <div class="toast">
            Tip: <span>Distance × difficulty multiplier + coins × 20</span> = score. Hard hits harder. 🔥
          </div>
        ...
        <div class="score-box">
          <div class="hud-title" style="margin-bottom:4px;">Scoreboard (<span id="scoreboard-diff-label">Easy</span>)</div>
...
// dynamic lanes array (0..1 positions)
let lanes = [];
...
function buildAchievements(finalScore) {
  ...
  if (coinsThisRun >= 5) badges.push('💰 Coin Hoarder (5+)');
  if (coinsThisRun >= 10) badges.push('🏦 Bank Robber (10+)');
  if (flawlessRun && lives === 3) badges.push('🛡️ Untouchable (no crashes)');
  if (maxSpeedSeen >= 220) badges.push('⚡ Speed Demon (220km/h+)');
  if (finalScore >= 1000) badges.push('🌟 Neon Legend (1000+ score)');
  if (badges.length){
    achievementsLive.textContent = 'Achievements: ' + badges.join(' ');
  }
  return badges;
}
...
function handleCrash() {
  ...
  setTimeout(()=>{livesHearts.style.transform='scale(1)';},160);
  ...
  gameOverText.textContent =
    `Out of lives! ${currentDifficulty.toUpperCase()} – ${distance.toFixed(0)} m, ${coins} coins. Score: ${score}.`;
  achievementsSummary.textContent = badges.length
    ? 'Achievements: ' + badges.join(' | ')
    : 'Achievements: keep pushing for medals!';

  overlay.classList.add('show');

  const name = nameInput.value.trim() || 'Racer';
  const detail =
    `Neon Street Racer – diff=${currentDifficulty}, lanes=${lanes.length}, distance=${distance.toFixed(0)}m, coins=${coins}, score=${score}, badges=[${badges.join(', ')}]`;
...
async function loadScores(diff){
  ...
  try{
    ...
    let html = '<table><thead><tr><th>#</th><th>Player</th><th>Score</th><th>Dist</th><th>Coins</th></tr></thead><tbody>';
    rows.forEach((r, i)=>{
      html += `<tr>
        <td>${i+1}</td>
        <td>${r.player_name}</td>
